...
{
    public class FeatureGeneratorElementNode3DDouble
	{
		...
		public FeatureGeneratorElementNode3DDouble(
			int offset)
		{
            ...
            this.covariance_matrix = new double[9];
        ...
        public void GenerateFeaturesTree(IRaster3DInteger raster, IMaxTreeNode<float> bottom_level_node, double[,] features)
        {
			...
			double [,] features)
		{
			...
			// 1 get_size
			double size = node.CulmativeRealSize;
            ...
            double x_mean = features[node_index, IndexSX] / size;
            double y_mean = features[node_index, IndexSY] / size;
            double z_mean = features[node_index, IndexSZ] / size;
			...
			covariance_matrix[0] = (covariance_matrix[0] + size / 12.0f) / Math.Pow(size, 5.0 / 3.0);
			covariance_matrix[4] = (covariance_matrix[4] + size / 12.0f) / Math.Pow(size, 5.0 / 3.0);
			covariance_matrix[8] = (covariance_matrix[8] + size / 12.0f) / Math.Pow(size, 5.0 / 3.0);

			covariance_matrix[1] = covariance_matrix[1] / Math.Pow(size, 5.0 / 3.0);
			covariance_matrix[2] = covariance_matrix[2] / Math.Pow(size, 5.0 / 3.0);
			covariance_matrix[5] = covariance_matrix[5] / Math.Pow(size, 5.0 / 3.0);

			// CollectionTools.print(covariance_matrix);
			// 3 compute eigenvalues
			ToolsMathMatrix3Float64.eigenvalues_symetric_fill(covariance_matrix, eigenvalues);
			...
			// 5 compute d values
			double d0 = Math.Sqrt((eigenvalues[0] * 20));
			double d1 = Math.Sqrt((eigenvalues[1] * 20));
			double d2 = Math.Sqrt((eigenvalues[2] * 20));

...
namespace KozzionMathematics.Statistics.Distribution.Implementation
{
    ...
    // */
    public class DistributionKolmogorovSmirnov
    {
         ...
         */
        public DistributionKolmogorovSmirnov(int n)
        {
            if (n <= 0)
            {
                throw new Exception("n must be postive");
            }
            this.n = n;
        }
        ///**
        ...
        public double ComputeCDF(double d)
        {
            return this.ComputeCDF(d, false);
        }
        ...
        // * {@code 0 <= h < 1}.
        // */
        public double ComputeCDFExact(double d)
        {
            return this.ComputeCDF(d, true);
        }
        ...
        // * {@code 0 <= h < 1}.
        // */
        public double ComputeCDF(double d, bool exact)
        {
        ...
        //  * {@code 0 <= h < 1}.
        //  */
        private double ComputeExactK(double d)
        {
            int k = (int)Math.Ceiling(n * d);
            BigDecimal[,] H = this.createH(d);
            throw new NotImplementedException("Can't power the array properly");
            //BigDecimal[,] Hpower = H.power(n);
            //BigDecimal pFrac = Hpower[k - 1, k - 1];
            //for (int i = 1; i <= n; ++i)
            //{
            //    pFrac = (pFrac * i) / n;
            //}

...
import com.kozzion.library.math.statistics.distribution.implementation.NormalDistributionDouble;
import com.kozzion.library.plotting.utility.PlottingTools;

public class NaiveBayesClassifierContinuous 
{
	private NormalDistributionDouble[] churnDistributions;
	private NormalDistributionDouble[] nonChurnDistributions;
	
	public NaiveBayesClassifierContinuous(double[][] data, int[] churn, double threshold, String[] cat)
	{
		...
		for (int i=0;i<churners.length;i++)
			if (churners[i][0]!=0) l.add(churners[i][0]);
		INumberDistribution<Double> generate_distribution = sdf.generate_distribution(l);*/
		...
		churnDistributions = new NormalDistributionDouble[averages.length];
		for (int i=0;i<averages.length;i++)
			churnDistributions[i] = new NormalDistributionDouble(averages[i], deviations[i]);
		...
		nonChurnDistributions = new NormalDistributionDouble[averages.length];
		for (int i=0;i<averages.length;i++)
			nonChurnDistributions[i] = new NormalDistributionDouble(averages[i], deviations[i]);
	...
	public int classify(double[] instance)
	{
		...
		for (int i=0;i<instance.length;i++)
		{
			if (instance[i]!=0)
			{
				nonChurnChance *= nonChurnDistributions[i].get_probability_density(instance[i]);
				churnChance *= churnDistributions[i].get_probability_density(instance[i]);
	...
	public int classify2(double[] instance)
	{
		double distanceChurn = 0;
		double distanceNonChurn = 0;
		
		for (int i=0;i<instance.length;i++)
		{
			distanceChurn+=Math.abs(churnDistributions[i].get_mean()-instance[i]);
			distanceNonChurn+=Math.abs(nonChurnDistributions[i].get_mean()-instance[i]);
		}
		
		if (distanceChurn<(distanceNonChurn*threshold)) return 1;
		return 0;
	}
